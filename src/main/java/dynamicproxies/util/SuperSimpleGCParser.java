    ...
    public GCStatistics(String filename) {
      try (
        ...
        if (events.isEmpty()) {
          ...
          memoryReclaimedDuringYoung =
              events.stream().filter(Event::isYoung).mapToLong(Event::getMemoryReclaimed).sum();
          ...
          averateTimeInYoungGCs =
              events.stream().filter(Event::isYoung).mapToDouble(Event::getTimeInGC).mapToInt(time -> (int) time).summaryStatistics();
          ...
          totalMemoryAllocated =
              events.stream().mapToLong(Event::getMemoryReclaimed).sum() + lastEvent.getHeapAfterGC();
